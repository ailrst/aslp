...
module EmbeddingLanguage = struct 
  (* For splitting up functions we move the mutability property to the expression type so we 
     know how to refer to variables in a function call context.
    (we could also just pass bindings / the mutable IdentSet) *)
