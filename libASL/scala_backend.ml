 ...
 *)
  ...
  class branch_outliner (funsig: Eval.fun_sig) (outline_thresh:int) = object(this)
    ...
    val scoped_bindings : ty Transforms.ScopedBindings.t  = let x = Transforms.ScopedBindings.init () in
      List.iter (fun (t,i) -> add_bind x i t) (fnsig_get_typed_args funsig);
      push_scope x ();
      x
    
    val return_type = fnsig_get_rt funsig
    val mutable extra_funs : Eval.fun_sig Bindings.t = Bindings.empty

    method add_fun (bs: (ident * Eval.fun_sig) list) = extra_funs <- Bindings.union 
    ...
    method! vstmt s =
      match s with
        | Stmt_VarDeclsNoInit(ty, vs, loc) ->
            List.iter (fun f -> add_bind scoped_bindings f ty) vs; 
            DoChildren
        | Stmt_VarDecl(ty, v, i, loc) ->
            add_bind scoped_bindings v ty ;
            DoChildren
...
let write_fn (name: AST.ident) (ret_tyo, argtypes, targs, args, l, body) st = 
  ...
  Bindings.iter (fun n b -> print_write_fn n b st) to_add
